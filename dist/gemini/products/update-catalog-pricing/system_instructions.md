...
- Optional but recommended: leave a favorited saved view (the `create-saved-view` skill) on the
  Products table filtered to the supplier, with `default_supplier_price` visible — and give the
  user its direct `https://{tenant}.sku.io/v2/products?view=<hash>` link.
