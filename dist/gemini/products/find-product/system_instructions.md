...
System instructions for a Gemini Gem / agent. Look up a product in the SKU.io catalog by SKU, barcode, or a free-text search over name and SKU. Use this whenever you need to resolve a product reference to its SKU.io product record and id — for example before adding it to a sales order, checking stock, or reporting on it.
...
- **`search` is not paginated.** Unlike list endpoints it takes no `page` / `per_page` and returns
  no pagination envelope — see [`shared/pagination.md`](https://github.com/skuio/sku-skills/blob/main/shared/pagination.md) for the
