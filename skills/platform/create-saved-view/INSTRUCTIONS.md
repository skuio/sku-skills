...
2. **`query_data`** — a **JSON string** describing the view state:
   ...
   table** — reuse the keys the page already exposes; don't invent them.
...
4. **Create + favorite in one call:**
   ...
   curl -sS -X POST "https://$SKU_TENANT.sku.io/api/data-tables/saved-views" \
     ...
     -d '{
       ...
       "query_data": "{\"search\":\"The Whale Lounge\",\"sortBy\":\"-created_at\",\"columns\":{\"visible\":[\"image\",\"sku\",\"name\",\"brand_name\",\"barcode\",\"default_supplier_sku\",\"unit_cost\",\"default_price\",\"created_at\"]},\"pagination\":{\"per_page\":50}}",
...
- **Valid keys only.** Column and filter keys must be ones the table exposes — an unknown column key
  ...
  know exist.
- **Custom attributes usually aren't columns.** Ad-hoc product attributes (e.g. an imported "Carton
  Pack") generally don't have a data-table column; a saved view surfaces standard columns and
  filters. To review attribute values, open a product's detail page.
